...
- Java 17
- Gradle Wrapper 사용
- 기본 실행 DB: H2 In-Memory Database
- 별도 MySQL 설치 없이 로컬 실행 가능
- 기본 서버 포트: 8080
- H2 Console: `/h2-console`

> `src/main/resources/data.sql`을 통해 메뉴 및 테스트용 초기 데이터가 주입됩니다.
...
- 최초 포인트 row가 없는 사용자에게 동시에 충전 요청이 들어오는 경우, `points.user_id`의 unique 제약으로 중복 생성을 방지합니다. 운영 환경에서는 unique 제약 위반 발생 시 재조회 후 재시도하는 전략을 추가해 안정성을 높일 수 있습니다.

- 현재 주문 데이터 수집 플랫폼 전송은 `@TransactionalEventListener(AFTER_COMMIT)`와 Mock Client로 처리했습니다. 과제 요구사항에서는 Mock API 또는 테스트 코드로 전송 검증이 가능하다고 판단했기 때문에 별도의 메시지 큐나 Outbox 패턴은 도입하지 않았습니다. 다만 실제 외부 API 연동 환경에서는 전송 실패, 네트워크 장애, 재처리 문제가 발생할 수 있으므로 Outbox 패턴 또는 메시지 큐를 도입해 전송 실패 재처리를 보완할 수 있습니다.

- 인기 메뉴 조회는 최근 7일간의 `OrderItem`을 DB에서 직접 집계합니다. 현재 과제 규모에서는 정확성을 우선해 별도 캐시나 집계 테이블을 두지 않았지만, 주문 데이터가 많아질 경우 매 요청마다 group by 집계가 병목이 될 수 있습니다. 추후에는 Redis Sorted Set, 캐싱, 또는 스케줄러 기반 인기 메뉴 집계 테이블을 도입해 조회 성능을 개선할 수 있습니다.
